...
import { APPLY_HTTP_GET, APPLY_HTTP_POST } from '@store/actions/util/http';
...
import { applyHttpGet, applyHttpPost } from '@utils/client/http';
...
function* invokeApplyHttpPost(action) {
  const { url, options, callback } = action.payload;
  const response = yield call(applyHttpPost, url, options);
  switch (response.status) {
    case 200:
      yield put(applyStatus({ status: appStatuses.SUCCESS }));
      yield callback(response.data);
...
function* watchAsyncTriggers() {
  ...
  yield takeEvery(APPLY_HTTP_POST, invokeApplyHttpPost);
}
